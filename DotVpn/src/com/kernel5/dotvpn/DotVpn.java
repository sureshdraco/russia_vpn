...
public class DotVpn extends Activity {
    ...
    protected int _splashTime = 1000; // time to display the splash screen in ms
                                    // the first time, needs to download some object ( at least 1 )
...
//    private Context mContext;
    ...
    public void onCreate(Bundle savedInstanceState) {
      ...
      Constants.rrFont = Typeface.createFromAsset(getAssets(),Constants.FONT_RR);
      Constants.osFont = Typeface.createFromAsset(getAssets(),Constants.FONT_SO);
...
//      this.mContext = this.getApplicationContext();
      ...
      Log.v( TAG, "Screen pixels : " + this.mMetrics.widthPixels + "x" + this.mMetrics.heightPixels );
      Log.v( TAG, "Supposed size : " + this.mMetrics.widthPixels/this.mMetrics.densityDpi + "x" + 
                                       this.mMetrics.heightPixels/this.mMetrics.densityDpi + " inches" );
...
//    		  this.getApplicationContext().getSharedPreferences(Constants.AppName,MODE_WORLD_READABLE);
      ...
      Thread splashTread = new Thread() {
          ...
          public void run() {
              try {
                  int waited = 0;
                  while( DotVpn.this._active && waited < DotVpn.this._splashTime ) {
                    sleep(250);
    ...
    public void showSplashScreen(boolean force)
    {
         ...
         this.mIv = new ImageView[] { (ImageView) findViewById(R.id.SplashProgressA),
    ...
    public void animProgress()
    {
        ...
        DotVpn.this.mIv[DotVpn.this.mCurAnim].setBackgroundResource(R.drawable.loadingb);
    }
