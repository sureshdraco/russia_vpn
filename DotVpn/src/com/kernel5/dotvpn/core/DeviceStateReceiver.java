...
import static com.kernel5.dotvpn.core.OpenVPNManagement.pauseReason;

public class DeviceStateReceiver extends BroadcastReceiver implements
		ByteCountListener {
	...
	public void networkStateChange(Context context) {
		...
		if (networkInfo == null) {
			 ...
			 * = wifiMgr.getConnectionInfo(); extrainfo+=wifiinfo.getBSSID();
			 * 
			 * subtype += wifiinfo.getNetworkId(); }
			 */

			netstatestring = String.format("%2$s %4$s to %1$s %3$s",
					networkInfo.getTypeName(), networkInfo.getDetailedState(),
					extrainfo, subtype);
		}
		...
		if (networkInfo != null && networkInfo.getState() == State.CONNECTED) {
			...
			if (sendusr1 && lastNetwork != newnet) {
				...
				if (shouldBeConnected()) {
					Log.d("suresh", String.valueOf(true));
					if (lastNetwork == -1) {
						mManagement.resume();
					} else {
					...
					Log.d("suresh", String.valueOf(false));
				}
				lastNetwork = newnet;
			}
	...
	private boolean shouldBeConnected() {
		return (screen == connectState.SHOULDBECONNECTED
				&& userpause == connectState.SHOULDBECONNECTED && network == connectState.SHOULDBECONNECTED);
	}
