...
public class ConnectActivity extends Activity implements StateListener, ByteCountListener, AdapterView.OnItemSelectedListener, View.OnClickListener, SharedPreferences.OnSharedPreferenceChangeListener {

	private final static boolean DEBUG = false;
	
    private static final String TAG = Constants.TAG;
    private static final int UPDATE_PERIOD = 1000; // update state period (in ms)
...
//    protected boolean mConnectionPending = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
    	...
    	this.selected_server_url = this.mPrefs.getString(getString(R.string.SettingsKeyServerUrl), Constants.SERVER_NAME_DE);
    	this.selected_server_config = this.mPrefs.getString(getString(R.string.SettingsKeyServerConfigFileName), Constants.VPN_CONFIG_DE);
    	// setup listener
      ...
      try {
         ...
         if (DEBUG) Log.v( TAG, "Created traffic database connection" );
      } catch (Exception e) {
    	  if (DEBUG) Log.e( TAG, "Couldn't open the traffic database", e );
      }
      ...
      Runnable mGetUser = new Runnable() {
          ...
          public void run() {
            ...
            try {
               ...
               request.token = ConnectActivity.this.mPrefs.getString( Constants.OAUTH_TOKEN, "" );
               ...
               if ( response.code == 0 )
               {
                  ...
                  final String[] pemail = ConnectActivity.this.mUsername.split("@");
                  if ( pemail.length != 2 )
                  {
                  ...
                  String epass = ConnectActivity.this.mPrefs.getString( Constants.BAUTH_TOKEN, "" );
            	...
            	if (DEBUG) Log.v( TAG, "Couldn't get user info", e );
            }
          }
      };
      Thread t  = new Thread( mGetUser );
      t.start();
      if (DEBUG) Log.v( TAG, "Waiting user info ... " );
      try {
          t.join();
      } catch ( Exception e ) {
    	  if (DEBUG) Log.v( TAG, "Getting user threadus interruptus", e );
      }
    ...
    public void showConnectScreen(boolean force)
    {
         ...
         LocationSpinnerAdapter adapter = new LocationSpinnerAdapter(this, R.layout.country_spinner_layout, 
        		 													 new String[] { "Germany",
																	        		 "France",
																	        		 "Japan",
																	        		 "Netherlands",
         ...
         this.mTrafficView.setup(this,
                            this.mTrafIn, this.mTrafOut,
                            getString(R.string.traffic_title_in ),
                            getString(R.string.traffic_title_out ),
                            this.mLHor, this.mLVer);
...
//			if ( this.mConnectionPending ) return;
	...
	public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
		...
		preferences_editor.putString(getString(R.string.SettingsKeyServerUrl), this.selected_server_url);
		preferences_editor.putString(getString(R.string.SettingsKeyServerConfigFileName), this.selected_server_config);
		preferences_editor.commit();
		////////////////////////////////////////////////////////////////

		if (DEBUG) Log.i(TAG, "Connect to " + country + " (" + pos + ")");
    ...
    public void updatePrivateIp()
    {
    	...
    	try {
          one: for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements();) {
           ...
           if (DEBUG) Log.e(TAG, "Could not find private IP", e);
    	}
        ...
        Runnable r = new Runnable() {
         ...
         public void run() {
          try {
             ...
             ConnectActivity.this.mPublicIp = EntityUtils.toString(entity).replaceAll("\"", "" );

             runOnUiThread(new Runnable() {
                @Override
                public void run() {
                	ConnectActivity.this.public_ip_view.setText( ConnectActivity.this.mPublicIp );
                }
             });
          } catch (Exception e) {
             if (DEBUG) Log.w(TAG, "Could not update public IP - " + e.getMessage());
             // apparently we can reach the service behind the url -> show "no connection"
             runOnUiThread(new Runnable() {
                 @Override
                 public void run() {
                	 ConnectActivity.this.public_ip_view.setText("no connection");
                 }
             });
          }
         }
           ...
           if ( OpenVPN.mLastLevel == ConnectionStatus.LEVEL_CONNECTED )
           {
            if ( ConnectActivity.this.mPublicIp.contains(".") )
            {
               if ( ConnectActivity.this.mPing == null )
               {
                  ...
                  try {
                    String GetURL = "https://" + ConnectActivity.this.selected_server_url;
                    if (DEBUG) Log.v( TAG, "trying to reach : " + GetURL );
                    HttpParams httpParameters = new BasicHttpParams();
                    int timeoutConnection = 3000;
                    HttpConnectionParams.setConnectionTimeout(httpParameters, timeoutConnection);
                    int timeoutSocket = 10000; // 10 seconds
                    ...
                    // even if certificate is wrong, we don't care here
                	  if (DEBUG) Log.w( TAG, "Couldn't reach server", e );
                  } finally {
                    endTimeMs = System.currentTimeMillis();
                  }
   
                  ConnectActivity.this.mPing = ((endTimeMs-startTimeMs)>0)? 
                          "" + (endTimeMs-startTimeMs) + " " + getString( R.string.millis ):
               ...
               try {
                   ...
                   IpResponse response = JacksonRequests.postForJson( Constants.IP_URL,
                                         request, IpResponse.class );
                   if ( response.code == 0 )
                   {
                      mDistance = ""+response.distance;
                   }
               } catch ( Exception e ) {
            	   if (DEBUG) Log.v( TAG, "Couldn't get distance to server", e );
               } 
               final String pDistance = mDistance + " " + getString( R.string.kilometers );

               runOnUiThread(new Runnable() {
                @Override
                public void run() {
//                 TextView sl = (TextView) findViewById( R.id.sl );
//                 ImageView sli = (ImageView) findViewById( R.id.sli );
//                 if ( sl != null ) 
//                 {
//                    sl.setText( sCountryCode );
//                    if ( !sCountryCode.equals( getString( R.string.unknown ) ) )
//                    {
//                       lp = new RelativeLayout.LayoutParams( 40, 25);
...
//                 }
                 ...
                 if ( pi != null ) pi.setText( ConnectActivity.this.mPing );
                 TextView ds = (TextView) findViewById( R.id.ds );
                 if ( ds != null ) ds.setText( pDistance );
                 TextView pt = (TextView) findViewById( R.id.pt );
                 if ( pt != null ) pt.setText( R.string.openvpn );
                 TextView fw = (TextView) findViewById( R.id.fw );
                 if ( fw != null ) { 
                	 fw.setText( getString( R.string.active ) );
                	 fw.setCompoundDrawablesRelativeWithIntrinsicBounds(R.drawable.check_active, 0, 0, 0);
                 }
                }
               });
            }
           ...
           else
           {
               runOnUiThread(new Runnable() {
                ...
                public void run() {
//                 TextView sl = (TextView) findViewById( R.id.sl );
//                 if ( sl != null ) sl.setText( R.string.unknown )  );
//                 sl.setPadding( 0, 0, 0, 0 );
//                 ImageView sli = (ImageView) findViewById( R.id.sli );
//                 sli.setVisibility( View.GONE );
                 TextView pi = (TextView) findViewById( R.id.pi );
                 if ( pi != null ) pi.setText( R.string.unknown );
                 TextView ds = (TextView) findViewById( R.id.ds );
                 if ( ds != null ) ds.setText( R.string.unknown );
                 TextView pt = (TextView) findViewById( R.id.pt );
                 if ( pt != null ) pt.setText( R.string.unknown );
                 TextView fw = (TextView) findViewById( R.id.fw );
                 if ( fw != null ) { 
                	 fw.setText( R.string.inactive );
                	 fw.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, 0, 0);
                 }
                }
               });
           }
          } catch (Exception e) {
        	  if (DEBUG) Log.e(TAG, "Could not update server info", e);
          }
         }
        };
        Thread t = new Thread(r);
    ...
    public void updateTraffic()
    {
      if ( this.mTrafficSource != null )
      {
        try { 
          ...
          if ( max[0] == 0 ) max[0]=64*1024;

          // set scale
          long step = max[0] / (Constants.NB_GRAPH_LABELS-1);  
          for ( int li=0; li<Constants.NB_GRAPH_LABELS; li++ )
          {
              this.mLVer[li] = OpenVpnService.humanReadableByteCount( (max[0]-step*li), false );
          }
          ...
          runOnUiThread(new Runnable() {
            @Override
            public void run() {
            	if (DEBUG) Log.v( TAG, "Updating traffic" );
              ConnectActivity.this.mTrafficView.setData( ConnectActivity.this.mTrafIn, ConnectActivity.this.mTrafOut,
                            getString(R.string.traffic_title_in ) + " : " + 
                            OpenVpnService.humanReadableByteCount( totalIn[0], false ),
                            getString(R.string.traffic_title_out ) + " : " + 
                            OpenVpnService.humanReadableByteCount( totalOut[0], false ),
                            ConnectActivity.this.mLHor, ConnectActivity.this.mLVer);
            }
         });

        } catch ( Exception e ) {
        	if (DEBUG) Log.e( TAG, "Could not update traffic", e );
        } 
      }
    }
    ...
    private Runnable updateUI = new Runnable() {
		...
		public void run() {
			if (ConnectActivity.this.connection_dots.getVisibility() == View.VISIBLE) {
			    ...
			    ConnectActivity.this.connection_dots.setImageResource( getRId(ConnectActivity.this.mCounter) );
			}
		    if ( ConnectActivity.this.mHandler != null )
		    {
		    	ConnectActivity.this.mHandler.postDelayed( this, UPDATE_UI_PERIOD );
		    }
		}
...
//		this.mConnectionPending=true;
		...
		try {
	         ...
	         try {
	        	 if (DEBUG) Log.v( TAG, "Inserting tun module" );
	            executeSUcmd("insmod /system/lib/modules/tun.ko");
	         } catch ( Exception e ) {
	        	 if (DEBUG) Log.e( TAG, "Could not insert tun module", e );
	         }
	
	         try {
	        	 if (DEBUG) Log.v( TAG, "Changing tun permission" );
	            executeSUcmd("chown system /dev/tun");
	         } catch ( Exception e ) {
	        	 if (DEBUG) Log.e( TAG, "Could not change tun permissions", e );
	         }
	
	         if (intent != null) {
	            // Start the query
	            try {
	            	if (DEBUG) Log.v( TAG, "starting android vpn service" );
	                startActivityForResult(intent, START_VPN_PROFILE);
	            } catch (ActivityNotFoundException ane) {
	                // Shame on you Sony! At least one user reported that
	                // an official Sony Xperia Arc S image triggers this exception
	            	if (DEBUG) Log.e( TAG, "Could not start VPN service", ane );
	            }
    ...
    {
       try {
         if (this.mService != null && this.mService.getManagement() != null) this.mService.getManagement().stopVPN();
         else throw new Exception( "service is not bound" );
       }catch(Exception e){
    	   if (DEBUG) Log.e(TAG,"Couldn't stop VPN service",e);
       }
    }
    ...
    private void executeSUcmd(String command) throws Exception {
        ...
        if(ret!=0) 
        {
        	if (DEBUG) Log.v( TAG, "su command returned : " + ret );
    ...
    protected void onActivityResult (int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode==START_VPN_PROFILE) {
            if(resultCode == Activity.RESULT_OK) {
            	if (DEBUG) Log.v( TAG, "Vpn system service started" );
    ...
    protected class startOpenVpnThread extends Thread {
        @Override
        public void run() {
            VPNLaunchHelper.startOpenVpn(getBaseContext(), ConnectActivity.this.selected_server_config);
        }
    }
    ...
    public void updateState(final String status, final String logmessage, final int resid, final ConnectionStatus level) {
        ...
        if (DEBUG) Log.v( TAG, "Update state : " + status );
        
        if (status.equals("CONNERROR") || status.equals("AUTH_FAILED"))
        {
            if (DEBUG) Log.v( TAG, "Post error on CONNERROR or AUTH_FAILED" );
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                case R.string.state_resolve:
                    ConnectActivity.this.mStatus.setText( getString( R.string.sconnecting ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_red_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.INVISIBLE );
                    break;
                case R.string.state_connected:
//                    ConnectActivity.this.mConnectionPending=false;
                    ConnectActivity.this.mStatus.setText( getString( R.string.sconnected ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_green_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.VISIBLE );
                    unblockConnectUI();
                    updatePrivateIp();
                    updatePublicIp();
                    updateServerInfo();
...
//                    ConnectActivity.this.mConnectionPending=false;
                    ConnectActivity.this.mStatus.setText( getString( R.string.sauthfailed ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_red_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.INVISIBLE );
                    unblockConnectUI();
                    break;
...
//                    ConnectActivity.this.mConnectionPending=false;
                    ConnectActivity.this.mStatus.setText( getString( R.string.sconnerror ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_red_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.INVISIBLE );
                    unblockConnectUI();
                    break;
...
//                    ConnectActivity.this.mConnectionPending=false;
                    ConnectActivity.this.mStatus.setText( getString( R.string.sexiting ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_red_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.INVISIBLE );
...
//                    ConnectActivity.this.mConnectionPending=false;
                    ConnectActivity.this.mStatus.setText( getString( R.string.sdisconnected ).toUpperCase() );
                    ConnectActivity.this.is.setImageResource( R.drawable.status_red_bg );
                    ConnectActivity.this.connection_dots.setVisibility( View.INVISIBLE );
                    ...
                    { 
                       try {
                         ...
                         if (DEBUG) Log.v( TAG, "Cannot clear traffic" );
                       }
                    }
                    break;
                }
	...
	public void onSharedPreferenceChanged(SharedPreferences shared_preferences, String key) {
		...
		if (key.equals(Constants.OAUTH_TOKEN) || key.equals(Constants.BAUTH_TOKEN)) {
			
			if (shared_preferences.getString(Constants.OAUTH_TOKEN, "").length() == 0 &&
					shared_preferences.getString(Constants.BAUTH_TOKEN, "").length() == 0) 
			{
